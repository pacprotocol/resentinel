...
if [ ! -f /sentinel/pacprotocol.conf ]; then
  if [ -z "$RPCUSER" -o -z "$RPCPASSWORD" -o -z "$RPCPORT" ]; then
    echo "When no /sentinel/pacprotocol.conf is present, you must at least set RPCUSER, RPCPORT and RPCPASSWORD environment variables"
    exit 1
  fi

  echo "" > /sentinel/pacprotocol.conf
  if [ -n "$RPCUSER" ]; then
    echo "rpcuser=${RPCUSER}" >> /sentinel/pacprotocol.conf
  fi
  if [ -n "$RPCPASSWORD" ]; then
    echo "rpcpassword=${RPCPASSWORD}" >> /sentinel/pacprotocol.conf
  fi
  if [ -n "$RPCPORT" ]; then
    echo "rpcport=${RPCPORT}" >> /sentinel/pacprotocol.conf
  fi
fi
...
if [ ! -f /sentinel/sentinel.conf ]; then
  ...
  echo "dash_conf=/sentinel/pacprotocol.conf" > /sentinel/sentinel.conf
  if [ -n "$RPCHOST" ]; then
